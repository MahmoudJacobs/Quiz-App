...
import { FieldText, SubmitBtn } from "../Input/InputField";
  ...
  const onSubmit = async (data: string) => {
    try {
      const response = await axios.post(
        `${getBaseUrl()}/api/auth/forgot-password`,
        data
        // {
        //   headers: {
        //     language: "ar",
      ...
      <section className="bg-slate-950 min-h-screen ">
        <div className="grid grid-cols-1 lg:grid-cols-2">
          <div className={`${style.resSrc} pt-3 ps-5 lg:mt-10 lg:ms-5 `}>
            <img src={logo} alt="" />
            <div className="mt-2">
              <h6 className="text-lime-300 mt-16 text-2xl">Forgot password</h6>
              ...
              <div className="me-5 mt-24">
                <form onSubmit={handleSubmit(onSubmit)}>
                  <FieldText
                    ...
                    control={control}
                  />
                  <SubmitBtn name="Send email" submitting={isSubmitting} />
                </form>
              </div>
            </div>
